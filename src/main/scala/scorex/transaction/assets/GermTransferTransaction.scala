...
case class GermTransferTransaction private(assetId: Option[AssetId],
  ...
  override val transactionType: TransactionType.Value = TransactionType.GermTransferTransaction
...
object GermTransferTransaction {

  val MaxAttachmentSize = 140
  val MaxAttachmentStringSize = base58Length(MaxAttachmentSize)


  def parseTail(bytes: Array[Byte]): Try[GermTransferTransaction] = Try {

    val signature = ByteStr(bytes.slice(0, SignatureLength))
    val txId = bytes(SignatureLength)
    require(txId == TransactionType.GermTransferTransaction.id.toByte, s"Signed tx id is not match")
  ...
  def create(assetId: Option[AssetId],
             ...
             feeAmount: Long,
             attachment: Array[Byte],
             signature: ByteStr): Either[ValidationError, GermTransferTransaction] = {
    if (attachment.length > GermTransferTransaction.MaxAttachmentSize) {
